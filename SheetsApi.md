...
|getDataType()|Promise|N/A|This function returns the promise to get the values in HEADER_VOCAB_TYPE|
|parseDataType(response, headers)|String[]|response: the response from getDataType.<br>headers: the headers that need to get the data type.|This function parse the response from getDataType and returns the data type for input headers.|
|parseVocab(response, headers)|String[]|response: the response from getDataType.<br>headers: the headers that need to get the vocabulary.|This function parse the response from getDataType and return the vocab of input headers. Header that has no vocabulary will have an undefined in the return array.|
...
|selectFromTableWhereConditions(response, returnCold, condtions, returnType)|String[][] or object[]|response: the response from getSheet.<br>returnCols: an array of the names of columns need to return, passing "*" will return all columns.<br>conditions: An array of conditions in AND logic. Each condition is an object with format: {header:"the name of a header", value:"the value to check for"}. For OR logic between conditions, put them into an array. A complete example would be [{header:"h1", value:"v1"},[{header:"h2",value:"v2"}, {header:"h3", value:"v3"}]}]|This function takes the response from the getSheet, parses the conditions and returns the wanted type of values.|
...
|rowUpdate(sheetValues, sheetName, colVal, conditions)|Promise or int|sheetValues: the whole set of values of the target sheet, including the headers.<br>sheetName: the target sheet name.<br>colVal: an object of value to be updated with format {header: "value"}.<br>conditions: same as conditions in selectFromTableWhereConditions|This function does things like the sql sentence 'update sheetName set colVal where conditions'. It returns a promise to update the first row that matches the conditions. If no match found, it will return -1|
|parseUpdate(response)|int|response: the response from update(inputRange, inputValues).|This function parses the response from update(inputRange, inputValues) or rowUpdate(...) and returns the number of rows updated.|
...
|batchUpdateTable(sheetValues, sheetName, colVal, conditions)|Promise|sheetValues: the whole set of values of the target sheet, including the headers.<br>sheetName: the target sheet name.<br>colVal: an object of value to be updated with format {header: "value"}.<br>conditions: <b>an array of conditions in OR logic</b>. Each condition is an object with format: {header:"the name of a header", value:"the value to check for"}. For AND logic, use rowUpdate multiple times.|This function does things like the sql sentence 'update sheetName set colVal where conditions'.|
